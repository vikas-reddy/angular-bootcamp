...
export class ViewsFilterDisplayComponent implements OnDestroy {
  ...
  @Input() set region(value: string) {
    this.filterFormGroup.get('region').setValue(value, { emitEvent: false });
  };
  @Input() set dateTo(value: number) {
    this.filterFormGroup.get('dateTo').setValue(moment(value).format('YYYY-MM-DD'), { emitEvent: false });
  };
  @Input() set dateFrom(value: number) {
    this.filterFormGroup.get('dateFrom').setValue(moment(value).format('YYYY-MM-DD'), { emitEvent: false });
  };
